...
import { getAsciiKey } from "../core/input/util.js";
...
const UI = {
    ...
    async start(options={}) {
        ...
        try {
            ...
            Log.Error("Couldn't fetch version: " + err);
 ...
 * ------v------*/
    ...
    keyInput(event) {
        ...
        for (let i = newLen - inputs; i < newLen; i++) {
            const character = newValue.charAt(i);
            const codePoint = character.codePointAt(0);
            const key = (codePoint >= 0x20 && codePoint <= 0x7e)
                ? getAsciiKey(character)
                : null;
            ...
            UI.rfb.sendKey(key.keysym, key.code);
            if (key.shift) {

...
createDisk() {
  ...
  if ! disabled "$ALLOCATE"; then
    ...
    if (( dataSize > available )); then
      ...
      error "Insufficient free disk space to create a $diskDesc of ${diskSpace/G/ GB} in $base, it has only $gb available..."
...
resizeDisk() {
  ...
  if ! disabled "$ALLOCATE"; then
    ...
    if (( required > available )); then
      gb=$(formatBytes "$available")
      error "Insufficient free disk space to resize $diskDesc to ${diskSpace/G/ GB} in $base, it has only $gb available.."
...
convertDisk() {
  ...
  if ! disabled "$ALLOCATE"; then
    ...
    if (( currentSize > available )); then
      gb=$(formatBytes "$available")
      error "Insufficient free disk space to convert $diskDesc to $destinationFmt in $base, it has only $gb available..."

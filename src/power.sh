...
boot() {
  ...
  if [ -s "$QEMU_PTY" ]; then
    if [ "$(stat -c%s "$QEMU_PTY")" -gt 7 ]; then
      info "Ubuntu started successfully, visit http://127.0.0.1:8006/ to view the screen..."
      return 0
    fi
  fi
...
ready() {
  ...
  [ -f "$STORAGE/ubuntu.boot" ] && return 0
  [ ! -s "$QEMU_PTY" ] && return 1

  # Ubuntu uses UEFI boot with GRUB
  local line="GNU GRUB"
  grep -Fq "$line" "$QEMU_PTY" && return 0

  # Also check for Ubuntu login prompt as indicator
  grep -Fq "ubuntu login:" "$QEMU_PTY" && return 0
...
finish() {
  ...
  if [ -s "$QEMU_PID" ]; then
    ...
    pid=$(<"$QEMU_PID")
    echo && error "Forcefully terminating Ubuntu, reason: $reason..."
    { kill -15 "$pid" || true; } 2>/dev/null
  ...
  if [ ! -f "$STORAGE/ubuntu.boot" ] && [ -f "$BOOT" ]; then
    # Create boot marker after install
    if ready; then
      local file="$STORAGE/ubuntu.boot"
      touch "$file"
...
_graceful_shutdown() {
  ...
  if ! ready; then
    info "Cannot send ACPI signal during Ubuntu setup, aborting..."
    finish "$code" && return "$code"
  fi
  ...
  while [ "$cnt" -lt "$QEMU_TIMEOUT" ]; do
    ...
    info "Waiting for Ubuntu to shutdown... ($cnt/$QEMU_TIMEOUT)"
